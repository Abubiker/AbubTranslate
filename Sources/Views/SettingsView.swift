...
struct SettingsView: View {
            ...
            VStack(spacing: 0) {
                settingsRow(label: "Theme") {
                    ...
                    .onChange(of: appearanceMode) { _, newValue in
                        // Через model.appearanceMode (не model.applyAppearance()
                        // напрямую) — иначе запись останется незамеченной для
                        // @Observable, и preferredColorScheme в Panel не обновится.
                ...
                // Итог проверки живёт секунду-две в статусе и потом
                // исчезает — дата последней проверки остаётся якорем,
                // что автопинг вообще работает.
                Text(model.localizedString("Current version: %@", UpdateChecker.shared.currentVersion)
                     + (UpdateChecker.shared.lastCheckDate.map {
                         " · " + model.localizedString("Last checked: %@", $0.formatted(date: .abbreviated, time: .omitted))
                     } ?? ""))
                    .footnoteMuted()
                if case .failed(let message) = UpdateChecker.shared.status {
                    Text(message)
                        .font(.system(size: DSTokens.metaSize))
                        .foregroundStyle(.orange)
                        .lineLimit(2)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    private var updateStatusLabel: some View {
        ...
        case .downloading:
            // Детерминированная полоса: dmg весит ~1 МБ и при спиннере
            // процесс выглядит как «ничего не происходит».
            HStack(spacing: DSTokens.sm) {
                ProgressView(value: UpdateChecker.shared.downloadProgress)
                    .frame(width: 64)
                Text(verbatim: model.localizedString("Downloading…")
                     + " \(Int((UpdateChecker.shared.downloadProgress * 100).rounded(.down)))%")
                    .font(.system(size: DSTokens.metaSize))
                    .foregroundStyle(.secondary)
            }
        case .installing:
        ...
        case .available(let version):
            Text(verbatim: model.localizedString("Update available (%@)…", version))
                .font(.system(size: DSTokens.metaSize, weight: .medium))
