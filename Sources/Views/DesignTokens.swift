...
/// Liquid Glass фон на macOS 26+ — общий для Settings и Panel.
/// Стеклу карточек нужен цветной фон — без него .glassEffect() почти не виден.
@available(macOS 26.0, *)
struct LiquidGlassBackground: View {
    @Environment(\.colorScheme) private var colorScheme
    /// Panel просил тёмный фон чуть светлее, чем у Settings — тот же
    /// градиент, только менее чёрная база.
    var lighterDark: Bool = false
    var body: some View {
        ZStack {
            colorScheme == .dark
                ? (lighterDark
...
struct PanelCardSurface: ViewModifier {
    func body(content: Content) -> some View {
        // Тот же принцип, что и CardSurface: на macOS 26+ настоящее стекло
        // рисует свою кромку/подсветку само — ручные border/highlight/shadow
        // ниже для этой ветки не нужны, дублировали бы и выглядели бы шумно.
        if #available(macOS 26.0, *) {
            content
                .glassEffect(.regular, in: RoundedRectangle(cornerRadius: DSTokens.radiusCard))
        } else {
