...
Apple ничего не отправляет в сеть. MyMemory, HuggingFace и Azure —
отправляют переводимый текст на сторонний сервер.

HuggingFace и Azure при неудаче (нет ключа, лимит, пара не поддерживается)
автоматически откатываются на MyMemory — тот же принцип, что и раньше.

Движок, токен HuggingFace, ключ/регион Azure и почта MyMemory применяются
только по кнопке «Сохранить» в настройках — остальные поля (языки, хоткеи,
тема, автозапуск) применяются сразу. Причина: смена движка посреди набора
ключа не должна перезапускать перевод по недописанным данным.

Azure собран строго по официальной документации Microsoft — живым запросом
не проверен: создание Azure-аккаунта требует карту, такое действие
