...
| Local OPUS | на устройстве | модель для пары языков скачивается заранее в настройках |
| MyMemory | облако | без ключа; 5 000 слов/сутки анонимно, 50 000 с указанной почтой |
| HuggingFace | облако | нужен токен — anonymous access закрыт с их стороны, ключ бесплатный: huggingface.co/settings/tokens |

Apple и Local OPUS ничего не отправляют в сеть. MyMemory и HuggingFace —
...
Local OPUS: модели скачиваются по одной, независимо от того, сколько выбрано
за раз. В память загружается только модель активной пары, выгружается через
минуту простоя.
...
│   ├── LocalNeuralTranslator.swift # инференс OPUS через CoreML
│   ├── ModelDownloader.swift       # скачивание моделей OPUS, очередь
