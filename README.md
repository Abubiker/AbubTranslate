...
Два способа перевести:

- **Выделите текст** в любом приложении и нажмите ⌥⇧T — панель откроется уже
  с переводом.
- **Откройте панель** кликом по иконке и вставьте текст в поле «Оригинал» —
  перевод запустится сам, через полсекунды после последнего изменения.

Язык оригинала определяется автоматически, направление выбирается само: текст
на одном языке пары переводится в другой. Язык вне пары — например финский при
паре русский/английский — уходит в тот из двух, который совпадает с языком
системы, а не в тот, что стоит в списке первым.

Клик по иконке ничего не переводит и не трогает буфер обмена.

🔊 — озвучить перевод, 📋 — скопировать результат, ⚙️ — настройки, ⇄ — своп
языков. Первый перевод на новый язык: пакет скачается, в панели виден статус
загрузки.
...
- Вставка текста в открытую панель переводится автоматически, без нажатия кнопки
