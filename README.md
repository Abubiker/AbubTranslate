...
| DeepL | облако | нужен API-ключ: developers.deepl.com, бесплатный тир — разовый кредит 1 млн символов (не пополняется, дальше платный Growth); ⚠️ **сам может блокировать запросы из отдельных регионов на уровне сети** |
...
5. ⚠️ **DeepL сам может блокировать запросы из отдельных регионов на
   уровне сети** — ограничение на стороне провайдера, не баг приложения.
