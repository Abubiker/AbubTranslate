...
Сборка подписана ad-hoc, поэтому при первом запуске macOS покажет
предупреждение: **Настройки → Конфиденциальность и безопасность → Открыть
всё равно**.
...
Подпись обязательна: без неё arm64-бинарь не запустится. Ad-hoc подпись меняет
code identity при каждой пересборке, поэтому выданный «Универсальный доступ»
после сборки придётся выдавать заново.
