...
Apple ничего не отправляет в сеть. MyMemory, HuggingFace, Azure и Google —
отправляют переводимый текст на сторонний сервер.

HuggingFace, Azure и Google при неудаче (нет ключа, лимит, пара не
поддерживается) автоматически откатываются на MyMemory — тот же принцип,
что и раньше.

Движок, токен HuggingFace, ключ/регион Azure, ключ Google и почта MyMemory
...
посреди набора ключа не должна перезапускать перевод по недописанным данным.
...
приложение за пользователя не выполняет. HuggingFace и MyMemory проверены
реальными запросами.
